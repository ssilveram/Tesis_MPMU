...
# :::                Scenario 4 - Metro + Proximidad plena                   :::
# :::                                                                        :::
# ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
#
# Nombre: 6_4_Accesibility_Scenario_4.R
#
...
# script Scn1, Scn2 y Scn3, en este Scn4 se evalúa la accesibilidad con la implementación del
# sistema de transporte de la primera línea del metro de Bogotá y la adopción del
# concepto de ciudad de proximidad o de ciudad de los 15 minutos en toda la ciudad.
#
...
# 10. Cálculo de ganancia de accesibilidad entre escenarios
# 11. Carga de datos del Script y detención de r5r y Java
# 11. Tiempo de procesamiento del script
#
Start_Time_S6_4_P0 <- Sys.time()
...
list.files(data_path_scenario_4)
pbf_file <- list.files(data_path_scenario_4, pattern = ".pbf")[1]
gtfs_file <- list.files(data_path_scenario_4, pattern = "zip")
pbf_file
gtfs_file

# Cargamos el motor de r5r
r5r_core_scenario_4 <- setup_r5(
  data_path = data_path_scenario_4
)
...
cat("Cores usados para r5r_core_Scn4:",r5r_core_scenario_4$getNumberOfThreads(), "cores.\n")
cat("Máxima memoria: ", maxMemory, "MB\n")
cat("Memoria total asignada: ", totalMemory, "MB\n")
cat("Memoria libre: ", freeMemory, "MB\n")

# Tiempo de procesamiento:
End_Time_S6_4_P1 <- Sys.time()
print(paste("1. Tiempo de inicialización: ", as.duration(End_Time_S6_4_P1 - Start_Time_S6_4_P1)))
...
# Tiempo de procesamiento:
End_Time_S6_4_P2 <- Sys.time()
print(paste("2. Tiempo de carga de datos de origen: ", as.duration(End_Time_S6_4_P2 - Start_Time_S6_4_P2)))
...
Shape_Isocrona_Ejemplo_Scn4 <- isochrone(
  r5r_core = r5r_core_scenario_4,
...
Shape_Isocrona_Ejemplo_Scn4
#st_area(Shape_Isocrona_Ejemplo_Scn4)

# Mostramos las Isocronas
tm_basemap("OpenStreetMap") +
  tm_shape(Shape_UPLs) +
  tm_polygons(fill = "white", fill_alpha = 0.001,  col_alpha = 0.25, col = "darkblue") +
  tm_shape(st_make_valid(Shape_Isocrona_Ejemplo_Scn4)) +
...
st_write(Shape_Isocrona_Ejemplo_Scn4, dsn = "./Data/2_Processing/6_Accesibility/Scn4/Shape_Isocrona_Ejemplo_Scn4.gpkg", driver = 'GPKG', append = FALSE)
Shape_Isocrona_Ejemplo_Scn4 <- st_read("./Data/2_Processing/6_Accesibility/Scn4/Shape_Isocrona_Ejemplo_Scn4.gpkg")
Shape_Isocrona_Ejemplo_Scn4

# Tiempo de procesamiento:
End_Time_S6_4_P3 <- Sys.time()
print(paste("3. Tiempo de cálculo de Isocronas: ", as.duration(End_Time_S6_4_P3 - Start_Time_S6_4_P3)))
...
Amenities_Puntos_Scn4_sf # Destinos (Puntos en proximidad plena)
...
Destinos_Accesibilidad_City_Scn4 <- Amenities_Puntos_Scn4_sf |>
...
Destinos_Accesibilidad_City_Scn4
names(Destinos_Accesibilidad_City_Scn4)

# Calculamos la accesibilidad.
# --- ¡ATENCIÓN! ¡Esto tarda 10,00 horas en procesarse! ---
# Acceso_a_Equipamientos_Scn4 <- accessibility(
#   r5r_core = r5r_core_scenario_4,
...
#   destinations = Destinos_Accesibilidad_City_Scn4, # |> filter(COD_UPL %in% c("15", "18")),
...
#   cutoffs = c(15, 30)
# )
head(Acceso_a_Equipamientos_Scn4)
Acceso_a_Equipamientos_Scn4 # 2556 / 1559916
length(Acceso_a_Equipamientos_Scn4$id) # 1559916
...
#fwrite(Acceso_a_Equipamientos_Scn4, "./Data/2_Processing/6_Accesibility/Scn4/Acceso_a_Equipamientos_Scn4_2025-10-22.csv", row.names = FALSE)
Acceso_a_Equipamientos_Scn4 <- as.data.frame(fread("./Data/2_Processing/6_Accesibility/Scn4/Acceso_a_Equipamientos_Scn4_2025-10-22.csv", sep=",", na = "null"))
Acceso_a_Equipamientos_Scn4

length(Acceso_a_Equipamientos_Scn4$id)
unique(Acceso_a_Equipamientos_Scn4$opportunity)

# Tiempo de procesamiento:
End_Time_S6_4_P4 <- Sys.time()
print(paste("4. Tiempo de cálculo de accesibilidad: ", as.duration(End_Time_S6_4_P4 - Start_Time_S6_4_P4)))
#Avisar()
...
Acceso_Equipamientos_MNZ_Scn4_sf <- Acceso_a_Equipamientos_Scn4 |>
  select(-percentile) |>
  pivot_wider(
  ...
  st_as_sf() |>
  arrange(COD_UPL)
Acceso_Equipamientos_MNZ_Scn4_sf # 43331 x 18 x 2 => 1559916/18 = 86662) <- 43331 (MNZ) x 2.

# Normalizamos entre 0 y 100 todos los valores máximos de cada columna:
Acceso_Equipamientos_MNZ_Scn4_15min_Norm_df <- Normalizacion(
  df = Acceso_Equipamientos_MNZ_Scn4_sf |> filter(cutoff == 15) |> st_drop_geometry(),
  Columna_Interes = c("COD_MNZ", "COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Todo")
# Calculamos las medias de las columnas normalizadas:
Acceso_Equipamientos_MNZ_Scn4_15min_Norm_Medias_df <- Normalizacion(
  df = Acceso_Equipamientos_MNZ_Scn4_sf |> filter(cutoff == 15) |> st_drop_geometry(),
  Columna_Interes = c("COD_MNZ", "COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Medias")

# Normalizamos entre 0 y 100 todos los valores máximos de cada columna:
Acceso_Equipamientos_MNZ_Scn4_30min_Norm_df <- Normalizacion(
  df = Acceso_Equipamientos_MNZ_Scn4_sf |> filter(cutoff == 30) |> st_drop_geometry(),
  Columna_Interes = c("COD_MNZ", "COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Todo")
# Calculamos las medias de las columnas normalizadas:
Acceso_Equipamientos_MNZ_Scn4_30min_Norm_Medias_df <- Normalizacion(
  df = Acceso_Equipamientos_MNZ_Scn4_sf |> filter(cutoff == 30) |> st_drop_geometry(),
  Columna_Interes = c("COD_MNZ", "COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Medias")

# Unimos los df de 15 y 30 normalizados para tenerlos en un solo lugar:
Acceso_Equipamientos_MNZ_Scn4_Norm_sf <- bind_rows(
  Acceso_Equipamientos_MNZ_Scn4_15min_Norm_df,
  Acceso_Equipamientos_MNZ_Scn4_30min_Norm_df
  ) |>
  left_join(
...
save(
  ...
  file = "./Data/2_Processing/6_Accesibility/Scn4/Acceso_Normalizado_MNZ_Scn4.RData"
)
load("./Data/2_Processing/6_Accesibility/Scn4/Acceso_Normalizado_MNZ_Scn4.RData")

# Tiempo de procesamiento:
End_Time_S6_4_P5 <- Sys.time()
print(paste("5. Tiempo de ordenado y normalización del acceso por MNZ: ", as.duration(End_Time_S6_4_P5 - Start_Time_S6_4_P5)))
...
Acceso_Equipamientos_UPL_Scn4_sf <- Acceso_Equipamientos_MNZ_Scn4_sf |>
  st_drop_geometry() |>
  left_join(
...
# Normalizamos entre 0 y 100 todos los valores máximos de cada columna:
Acceso_Equipamientos_UPL_Scn4_15min_Norm_df <- Normalizacion(
  df = Acceso_Equipamientos_UPL_Scn4_sf |> filter(cutoff == 15) |> st_drop_geometry(),
  Columna_Interes = c("COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Todo")
# Calculamos las medias de las columnas normalizadas:
Acceso_Equipamientos_UPL_Scn4_15min_Norm_Medias_df <- Normalizacion(
  df = Acceso_Equipamientos_UPL_Scn4_sf |> filter(cutoff == 15) |> st_drop_geometry(),
  Columna_Interes = c("COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Medias")

# Normalizamos entre 0 y 100 todos los valores máximos de cada columna:
Acceso_Equipamientos_UPL_Scn4_30min_Norm_df <- Normalizacion(
  df = Acceso_Equipamientos_UPL_Scn4_sf |> filter(cutoff == 30) |> st_drop_geometry(),
  Columna_Interes = c("COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Todo")
# Calculamos las medias de las columnas normalizadas:
Acceso_Equipamientos_UPL_Scn4_30min_Norm_Medias_df <- Normalizacion(
  df = Acceso_Equipamientos_UPL_Scn4_sf |> filter(cutoff == 30) |> st_drop_geometry(),
  Columna_Interes = c("COD_UPL", "NOM_UPL", "cutoff"),
  Tipo_Resultado = "Medias")

# Unimos los df de 15 y 30 normalizados para tenerlos en un solo lugar:
Acceso_Equipamientos_UPL_Scn4_Norm_sf <- bind_rows(
  Acceso_Equipamientos_UPL_Scn4_15min_Norm_df,
  Acceso_Equipamientos_UPL_Scn4_30min_Norm_df
  ) |>
  left_join(
...
save(
  ...
  file = "./Data/2_Processing/6_Accesibility/Scn4/Acceso_Normalizado_UPL_Scn4.RData"
)
load("./Data/2_Processing/6_Accesibility/Scn4/Acceso_Normalizado_UPL_Scn4.RData")

# Tiempo de procesamiento:
End_Time_S6_4_P6 <- Sys.time()
print(paste("6. Tiempo de ordenado y normalización del acceso por UPL: ", as.duration(End_Time_S6_4_P6 - Start_Time_S6_4_P6)))
...
for (Tiempo_Min in c(15, 30)) {
  ...
  Shape_Plotear_Acceso_MNZ <- Acceso_Equipamientos_MNZ_Scn4_sf |>
  ...
  for(i in 4:13){ # Columnas 4:13 o 15
    ...
    ggsave(
      ...
      device = NULL,
      path = "./Data/3_Results/6_Accesibility/Scn4/1_MNZ/",
      scale = 1,
...
# Tiempo de procesamiento:
End_Time_S6_4_P7 <- Sys.time()
print(paste("7. Tiempo de creación de mapas de accesibilidad por MNZ: ", as.duration(End_Time_S6_4_P7 - Start_Time_S6_4_P7)))
...
# ####
#
Start_Time_S6_4_P8 <- Sys.time()

# Evaluamos para 15 y 30 minutos mediante un ciclo para ahorrar trabajo:
for (Tiempo_Min in c(15, 30)) {
  
  # Alistamos el Shape a Plotear:
  Shape_Plotear_Acceso_UPL <- Acceso_Equipamientos_UPL_Scn4_sf |>
    filter(cutoff == Tiempo_Min) |>
  ...
  for(i in 3:12){ # Columnas 3:12 o 15
    ...
    ggsave(
      ...
      device = NULL,
      path = "./Data/3_Results/6_Accesibility/Scn4/2_UPL",
      scale = 1,
      width = 2160, # Intercambiadas porque está en vertical
      height = 3840,
      units = c("px"),
...
# Tiempo de procesamiento:
End_Time_S6_4_P8 <- Sys.time()
print(paste("8. Tiempo de creación de mapas de accesibilidad por UPL: ", as.duration(End_Time_S6_4_P8 - Start_Time_S6_4_P8)))
...
for (Tiempo_Min in c(15, 30)) {
  ...
  Shape_Plotear_HSQL_MNZ <- Acceso_Equipamientos_MNZ_Scn4_sf |>
    filter(COD_UPL %in% Lista_No_Rural$COD_UPL) |>
    filter(cutoff == Tiempo_Min) |>
  ...
  Shape_Plotear_HSQL_UPL <- Acceso_Equipamientos_UPL_Scn4_sf |>
    filter(COD_UPL %in% Lista_No_Rural$COD_UPL) |>
    filter(cutoff == Tiempo_Min) |>
  ...
  for(i in 3:9){ # Columnas 3:9
    ...
    ggsave(
      ...
      device = NULL,
      path = "./Data/3_Results/6_Accesibility/Scn4/3_MNZ_UPL/",
      scale = 1,
      width = 2160, # Intercambiadas porque está en vertical
      height = 3840,
      units = c("px"),
...
# Tiempo de procesamiento:
End_Time_S6_4_P9 <- Sys.time()
print(paste("9. Tiempo de creación de mapas de Funciones Sociales UPL+MNZ: ", as.duration(End_Time_S6_4_P9 - Start_Time_S6_4_P9)))
...
Diferencia_Accesibilidad_Scn1_Scn4 <- Acceso_a_Equipamientos_Scn4 |>
  group_by(id, cutoff) |>
  summarise(
    Total_Amenities_Scn4 = sum(accessibility)
  ) |>
  full_join(
    Acceso_a_Equipamientos_Scn1 |>
      group_by(id, cutoff) |>
  ...
  mutate(
    Diferencia_Scn4_Scn1 = Total_Amenities_Scn4 - Total_Amenities_Scn1
  ) |>
  ungroup() |>
  select(COD_MNZ = id, cutoff, Total_Amenities_Scn1, Total_Amenities_Scn4, Diferencia_Scn4_Scn1)
Diferencia_Accesibilidad_Scn1_Scn4

# Evaluamos para 15 y 30 minutos mediante un ciclo para ahorrar trabajo:
for (Tiempo_Min in c(15, 30)) {
  
  # Manzanas con diferencias:
  Manzanas_Nueva_Accesibilidad_Scn4 <- Diferencia_Accesibilidad_Scn1_Scn4 |>
    filter(cutoff == Tiempo_Min & Diferencia_Scn4_Scn1 > 0)
  Manzanas_Nueva_Accesibilidad_Scn4
  
  # Número de manzanas con Nueva Accesibilidad
  nrow(Manzanas_Nueva_Accesibilidad_Scn4)
  
  # Número total de equipamientos con nueva accesibilidad:
  sum(Manzanas_Nueva_Accesibilidad_Scn4$Diferencia_Scn4_Scn1)
  
  # Acceso medio incrementado por manzana
  Ganancia_Media_Accesibilidad_Scn4 = floor(sum(Manzanas_Nueva_Accesibilidad_Scn4$Diferencia_Scn4_Scn1)/nrow(Manzanas_Nueva_Accesibilidad_Scn4))
  Ganancia_Media_Accesibilidad_Scn4
  
  cat("\nCon la implementación de equipamientos en proximidad:\n",
      nrow(Manzanas_Nueva_Accesibilidad_Scn4), "manzanas han incrementado su accesibilidad en una media de\n",
      Ganancia_Media_Accesibilidad_Scn4, "equipamientos por manzana, para viajes de", Tiempo_Min, "minutos.\n")
  ...
  breaks_deciles <- quantile(Manzanas_Nueva_Accesibilidad_Scn4$Diferencia_Scn4_Scn1, probs = seq(0, 1, by = 0.1), na.rm = TRUE)
  breaks_deciles
  ...
  datos_bogota_deciles <- Manzanas_Nueva_Accesibilidad_Scn4 %>%
    select(COD_MNZ, Diferencia_Scn4_Scn1) |>
    mutate(
      accesibilidad_decil = cut(
        Diferencia_Scn4_Scn1,
  ...
  Shape_Plotear_Acceso_Scn4_Scn1 <- datos_bogota_deciles |>
    left_join(Shape_Manzanas) |>
    filter(COD_UPL %in% Lista_No_Rural$COD_UPL) |>
    st_as_sf()
  Shape_Plotear_Acceso_Scn4_Scn1
  ...
  Mapa_Acceso_Scn4_Scn1 <- ggplot() +
    ...
    # La capa de interés:
    geom_sf(
      data = Shape_Plotear_Acceso_Scn4_Scn1, # |> filter(COD_UPL == "15"),
      col = "white",
      linewidth = 0.1,
    ...
    theme_void() +
    theme(legend.position = "none")
  # Mapa_Acceso_Scn4_Scn1
  
  Nombre_Archivo <- paste0("Mapa_Diferencia_Acceso_MNZ_Scn4_Scn1_",Tiempo_Min,"min_BOG_2160x3840.png")
  Ruta_Archivo <- "./Data/3_Results/7_HQSL/2_Diferencia_Escenarios/"
  
  # Guardamos el mapa como imagen:
  ggsave(
    filename = Nombre_Archivo,
    plot = Mapa_Acceso_Scn4_Scn1,
    device = NULL,
    path = Ruta_Archivo,
    scale = 1,
    width = 2160, # Intercambiadas porque está en vertical
...
# Tiempo de procesamiento:
End_Time_S6_4_P10 <- Sys.time()
print(paste("10. Tiempo de diferencia de accesibilidad entre escenarios: ", as.duration(End_Time_S6_4_P10 - Start_Time_S6_4_P10)))
...
r5r_core_scenario_4$getNumberOfThreads()
r5r::stop_r5(r5r_core_scenario_4)
rJava::.jgc(R.gc = TRUE)

# Tiempo de procesamiento:
End_Time_S6_4_P11 <- Sys.time()
print(paste("11. Tiempo de detención de r5r y Java: ", as.duration(End_Time_S6_4_P11 - Start_Time_S6_4_P11)))
...
print(paste("8. Tiempo de creación de mapas de accesibilidad por UPL: ", as.duration(End_Time_S6_4_P8 - Start_Time_S6_4_P8)))
print(paste("9. Tiempo de creación de mapas de Funciones Sociales UPL+MNZ: ", as.duration(End_Time_S6_4_P9 - Start_Time_S6_4_P9)))
print(paste("10. Tiempo de cálculo de diferencia de accesibilidad entre escenarios: ", as.duration(End_Time_S6_4_P10 - Start_Time_S6_4_P10)))
print(paste("11. Tiempo de detención de r5r y Java: ", as.duration(End_Time_S6_4_P11 - Start_Time_S6_4_P11)))
print(paste("12. Tiempo de procesamiento del script:: ", as.duration(End_Time_S6_4_P0 - Start_Time_S6_4_P0)))
Avisar()
